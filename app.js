...
const roleCooldowns = {
    ...
    '1130958326847512646': 30000,  // 30 seconds
    '1130958325991866549': 10000   // 10 seconds
};
    ...
    if (message.author.id !== client.user.id && message.channel.id === '1130945796989272175') {
        ...
        if (authorRoles) {
            ...
            if (userCooldowns.has(authorId) && userCooldowns.get(authorId) > Date.now()) {
                ...
                setTimeout(async () => {
                    if (userMessage) await userMessage.delete().catch(() => { });
                }, 10000);
            }
        }
    }
...
client.on("presenceUpdate", async (oldPresence, newPresence) => {
    ...
    if (!member.user.bot) {
        const hasCustomStatus = newPresence.activities.some(activity => activity.name === "Custom Status" && activity.state &&activity.state.includes("NitroGen"));
        ...
        try {
            ...
            console.error(`Error while updating role ${roleId} for ${memberId}: ${err}`);
        }
    }
});

/**
 * Create a '.env' file with your Discord bot token like this:
 * T0KEN=YOUR_BOT_TOKEN
 * 
 * This script will then use this token to connect to the Discord API.
 */
